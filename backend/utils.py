...
def demo_safe_data_addition():
    ...
    # 添加商品
    new_product_id = generate_auto_id_df(data['product'], "商品ID")
    new_product_data = {
        "商品ID": [new_product_id],
        "货号": ["P001"],
        "类型": ["电子产品"],
        "图片路径": ["/images/product1.jpg"],
        "备注": ["测试商品"],
        "用途": ["测试用途"]
    }
    ...
    # 添加商品特征
    new_feature_id = generate_auto_id_df(data['feature'], "商品特征ID")
    new_feature_data = {
        "商品特征ID": [new_feature_id],
        "关联商品ID": [new_product_id],
        "单价": [299.99],
        "重量": [1.5],
        "规格": ["标准"],
        "材质": ["塑料"],
    ...
    result = add_data_to_csv({
        'product': new_product_df,
        'feature': new_feature_df
    })
